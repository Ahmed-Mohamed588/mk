...
if ( isset( $_POST['save_settings'] ) ) {
    ...
    $active_langs = ( isset( $_POST['active_langs'] ) && is_array( $_POST['active_langs'] ) ) ? array_map( 'sanitize_text_field', $_POST['active_langs'] ) : array();
    ...
    // تحديث القوائم
    if ( function_exists( 'mklang_register_nav_menus' ) ) mklang_register_nav_menus();
    ...
    <div class="mklang-card">
        <?php if ( $status === 'active' ): ?>
            <div style="background:#f9f9f9; padding:15px; border:1px solid #ddd; border-radius:5px; margin-bottom:20px; display:flex; justify-content:space-between; align-items:center;">
                ...
                <div><strong>الرصيد:</strong> <span style="font-size:20px; color:#46b450; font-weight:bold;">$<?php echo $credits; ?></span></div>
            </div>
            ...
            <form method="post" action="">
                            ...
                            <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap:10px;">
                                <?php foreach($all_languages as $code => $name): ?>
                                    ...
                                    <label style="background:#fff; border:1px solid #ddd; padding:5px 10px; display:flex; align-items:center; <?php echo $disabled ? 'opacity:0.5' : ''; ?>">
                                        <input type="checkbox" name="active_langs[]" value="<?php echo $code; ?>" <?php checked(in_array($code, $active_langs)); ?> <?php disabled($disabled); ?>>
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            <label><input type="checkbox" name="mklang_show_switcher" value="yes" <?php checked($show_switcher, 'yes'); ?>> تفعيل مبدل اللغات في الموقع</label>
                        </td>
                    </tr>
                    <tr>
                        <th>مكان الظهور</th>
                        <td>
                            <select name="mklang_switcher_loc">
                                <option value="shortcode" <?php selected($switcher_loc, 'shortcode'); ?>>استخدام Shortcode فقط [mklang_switcher]</option>
                                <option value="menu" <?php selected($switcher_loc, 'menu'); ?>>إضافة تلقائية للقائمة الرئيسية (Main Menu)</option>
                            </select>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="mklang_post_status">
                                <option value="draft" <?php selected($post_status, 'draft'); ?>>مسودة (Draft) - للمراجعة</option>
                                <option value="publish" <?php selected($post_status, 'publish'); ?>>نشر فوراً (Publish)</option>
                            </select>
                ...
                <p class="submit"><input type="submit" name="save_settings" class="button button-primary" value="حفظ التغييرات"></p>
            </form>

        <?php else: ?>
            <form method="post" action="">
                <?php wp_nonce_field( 'mklang_save_settings', 'mklang_nonce' ); ?>
                <p>أدخل مفتاح الترخيص: <input type="text" name="mklang_license" class="regular-text"></p>
                <p><input type="submit" name="submit_license" class="button button-primary" value="تفعيل"></p>
            </form>
        <?php endif; ?>
    </div>
</div>
...
jQuery(document).ready(function($){
    $('#default_lang').change(function(){
        ...
        $('input[name="active_langs[]"][value="'+val+'"]').prop('checked', false).prop('disabled', true).parent().css('opacity', '0.5');
    }).trigger('change');
});
</script>